"""Revised continuous Mountain Car training script.

Why this v2 script exists
-------------------------
The first continuous runs converged to near-zero actions and never solved the
control task. This version changes three things:
1. stronger defaults (especially for SAC),
2. a longer training budget,
3. an adapted reward setup that can penalize time as well as fuel.

Recommended workflow
--------------------
1. Solve the standard continuous task first:
...
2. Then run the adapted task with SAC first:
   python scripts/run_continuous_baselines_v2.py --scenarios continuous_fuel_binary --algorithms sac --timesteps 300000 --seeds 0 1 2 --cost-coef 0.05 --per-step-time-cost 0.10

Optional smoother proxy for debugging:
...
try:
    ...
    raise ImportError(
        "stable-baselines3 is not installed. Add 'stable-baselines3[extra]>=2.3.0,<3.0' "
        "to requirements.txt and run pip install -r requirements.txt first."
    ) from exc
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--algorithms", nargs="+", choices=["ppo", "sac"], default=["sac", "ppo"])
    parser.add_argument(
        "--scenarios",
        nargs="+",
        choices=["continuous_standard", "continuous_fuel_binary", "continuous_fuel_l1"],
        default=["continuous_standard"],
    )
    parser.add_argument("--timesteps", type=int, default=300_000)
    parser.add_argument("--eval-freq", type=int, default=10_000)
    parser.add_argument("--eval-episodes", type=int, default=50)
    parser.add_argument("--seeds", type=int, nargs="+", default=[0, 1, 2])
    parser.add_argument("--cost-coef", type=float, default=0.05)
    parser.add_argument("--non-null-threshold", type=float, default=0.05)
    parser.add_argument("--per-step-time-cost", type=float, default=0.10)
    parser.add_argument("--goal-bonus", type=float, default=100.0)
    parser.add_argument("--reward-mode", choices=["replace", "augment_original"], default="replace")
    parser.add_argument("--goal-velocity", type=float, default=0.0)
    parser.add_argument("--reset-low", type=float, default=-0.6)
    parser.add_argument("--reset-high", type=float, default=-0.4)
    parser.add_argument("--device", type=str, default="auto")
    parser.add_argument("--outdir", type=str, default="results/continuous_v2")
...
def make_env_factory(scenario: str, args: argparse.Namespace):
    ...
    if scenario == "continuous_standard":
        return lambda: make_continuous_env(adapted=False, **common_kwargs)
...
def main() -> None:
        ...
        for algo_name in args.algorithms:
            ...
            for seed in args.seeds:
                ...
                try:
                    ...
                    selected_model = model_cls.load(best_model_path, device=args.device) if best_model_path.exists() else model
                    ...
                    if eval_npz.exists():
                        plot_eval_curve_from_npz(eval_npz, title=f"{algo_name} | {scenario} | eval curve")
                    ...
                    plot_phase_portrait(trajectories, title=f"{algo_name} | {scenario} | greedy trajectories")
                    save_fig(run_dir / "phase_portrait.png")

                    plot_action_histogram(trajectories, title=f"{algo_name} | {scenario} | action histogram")
                    save_fig(run_dir / "action_histogram.png")
                    ...
                    rows.append(
                        {
                            ...
                            "eval_mean_time_cost": summary["mean_time_cost"],
    ...
    aggregate = (
        ...
        .agg(
            ...
            eval_mean_time_cost_mean=("eval_mean_time_cost", "mean"),
            eval_mean_non_null_count_mean=("eval_mean_non_null_count", "mean"),
            eval_mean_action_abs_mean=("eval_mean_action_abs", "mean"),
        )
